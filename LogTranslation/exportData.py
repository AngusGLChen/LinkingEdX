'''
This should be the first script to work with: it extract raw stackoverflow xml data into mysql (not postgres) database.
...
def main(type, op):
    ...
    if(type == "users"):
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/Users.xml'
    if(type == "questions"):
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/Posts.xml'
    if(type == "answers"):
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/Posts.xml'
    if(type == "closedquestionhistory" or type == "closedquestionhistory2" or type == "closedquestionhistory3"): # for closing question and edit questions respectively
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/PostHistory.xml'
    if(type == "votes"):
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/Votes.xml'
    if(type == "comments"):
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/Comments.xml'
    if(type == "badges"):
        infile = '/Volumes/NETAC/StackOverflow/Clear-out/Badges.xml'
    
    try:
        #CAMBIA CAMBIA CAMBIA CAMBIA   
        con = mysql.connector.connect(user='root', password='admin', host='127.0.0.1', database='StackOverflow')
        cur = con.cursor()
...
######################################
# types = ["users", "questions", "answers", "votes", "comments", "closedquestionhistory", "closedquestionhistory2", "closedquestionhistory3"]
#types = ["users", "questions", "answers"]

types = ["users", "questions", "answers", "comments"]
